document.addEventListener('DOMContentLoaded', () => {
    ...
    saveBtn.addEventListener('click', async () => {
        ...
        // Spécifiquement mettre à jour les statistiques annuelles modifiées
        const annualInputs = editorForm.querySelectorAll('input[name^="annual_"]');
        if (annualInputs.length > 0) {
            const year = annualInputs[0].name.split('_')[1]; // Récupérer l'année depuis le premier champ
            const yearData = currentEditingLayer.feature.properties.statistiques_annuelles.find(d => d.annee === year);
            if (yearData) {
                annualInputs.forEach(input => {
                    const key = input.name.split('_').slice(2).join('_');
                    const value = input.value;
                    // Conserver le type de la donnée originale (nombre ou texte)
                    if (typeof yearData[key] === 'number') {
                        yearData[key] = value === '' ? null : parseFloat(value);
                    } else {
                        yearData[key] = value;
                    }
                });
    ...
    function displayEditor(feature, layer, fileName, geojsonData) {
        ...
        for (const key in feature.properties) {
            ...
            let input;
            let currentValue = feature.properties[key];

            // Correction pour les données stockées en tant que chaîne JSON
            if ((key === 'statistiques_globales' || key === 'statistiques_annuelles') && typeof currentValue === 'string') {
                try {
                    currentValue = JSON.parse(currentValue);
                } catch (e) {
                    console.error(`Impossible de parser la propriété ${key} pour ${feature.properties.nom || 'point sans nom'}`);
                }
            }
            ...
            } else if (key === 'statistiques_globales') {
                const fieldset = document.createElement('fieldset');
                const legend = document.createElement('legend');
                legend.innerText = 'Statistiques Globales';
                fieldset.appendChild(legend);

                for (const statKey in currentValue) {
                    const statLabel = document.createElement('label');
                    statLabel.innerText = statKey.replace(/_/g, ' ');
                    const statInput = document.createElement('input');
                    statInput.type = 'text';
                    statInput.value = currentValue[statKey] || 'N/A';
                    statInput.readOnly = true;
                    fieldset.appendChild(statLabel);
                    fieldset.appendChild(statInput);
                }
                editorForm.appendChild(fieldset);
                continue; // Passer au champ suivant

            } else if (key === 'statistiques_annuelles' && Array.isArray(currentValue)) {
                const fieldset = document.createElement('fieldset');
                const legend = document.createElement('legend');
                legend.innerText = 'Statistiques Annuelles';
                fieldset.appendChild(legend);

                // Sélecteur d'année
                const yearSelectLabel = document.createElement('label');
                yearSelectLabel.innerText = 'Sélectionner une année';
                const yearSelect = document.createElement('select');
                yearSelect.id = 'year-select';
                currentValue.forEach(item => {
                    const option = document.createElement('option');
                    option.value = item.annee;
                    option.text = item.annee;
                    yearSelect.appendChild(option);
                });
                fieldset.appendChild(yearSelectLabel);
                fieldset.appendChild(yearSelect);

                const yearDataContainer = document.createElement('div');
                yearDataContainer.id = 'annual-stats-container';
                fieldset.appendChild(yearDataContainer);

                const renderAnnualStats = (yearData) => {
                    yearDataContainer.innerHTML = '';
                    for (const statKey in yearData) {
                        if (statKey === 'annee') continue;
                        const statLabel = document.createElement('label');
                        statLabel.innerText = statKey.replace(/_/g, ' ');
                        const statInput = document.createElement('input');
                        statInput.type = 'number';
                        statInput.name = `annual_${yearData.annee}_${statKey}`;
                        statInput.value = yearData[statKey] || 0;
                        yearDataContainer.appendChild(statLabel);
                        yearDataContainer.appendChild(statInput);
                    }
                };

                yearSelect.addEventListener('change', (e) => {
                    const selectedYear = e.target.value;
                    const yearData = currentValue.find(item => item.annee === selectedYear);
                    if(yearData) renderAnnualStats(yearData);
                });

                // Afficher les données de la première année par défaut
                if (currentValue.length > 0) {
                    renderAnnualStats(currentValue[0]);
                }
